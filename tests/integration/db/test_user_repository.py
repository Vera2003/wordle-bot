...
from src.domain.user import TelegramId, User, Username
...
class TestUserRepositorySave:
    ...
    async def test_update_existing_user(self, db_session: AsyncSession):
        ...
        user.add_points(50)
        user.use_energy()
        user.update_profile(username=Username("jane"), full_name="Jane Doe")
        await repository.save(user)
        await db_session.commit()
...
class TestUserRepositoryTransactions:
    ...
    @pytest.mark.skip(
        reason="SQLite in-memory doesn't enforce unique constraints in transactions"
    )
    @pytest.mark.asyncio
