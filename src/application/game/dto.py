...
from ..common.dto import BaseDTO, AuditMetadata, EntityId
...
class GameAttemptDTO(BaseDTO):
    ...
    points_earned: int = Field(default=0, ge=0, description="Заработано очков за эту попытку")
...
class SubmitGuessInput(BaseDTO):
    ...
    guess: str = Field(..., min_length=1, max_length=20, description="Угадываемое слово")
...
class GameStateOutput(BaseDTO):
    ...
    user_id: UUID = Field(..., description="ID пользователя")
    target_word: str = Field(..., description="Загаданное слово")  # На клиент не отправляется!
    attempts: List[GameAttemptDTO] = Field(default_factory=list, description="История попыток")
...
class SubmitGuessOutput(BaseDTO):
    ...
    game_result: Optional[GameResultOutput] = Field(None, description="Результат (если игра закончилась)")
