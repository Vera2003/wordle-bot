...
async def test_award_and_use_prize_flow(client, app_fixture):
    ...
    user_prizes_response = await client.get(f"/api/v1/prizes/users/{app_fixture.state.test_user_id}")
    assert user_prizes_response.status_code == 200
    assert len(user_prizes_response.json()) == 1

    unused_response = await client.get(f"/api/v1/prizes/users/{app_fixture.state.test_user_id}/unused")
